...
err_t tcp_server_send_data(void *arg, struct tcp_pcb *tpcb)
{
    ...
    if (state->running == start)
    {
        ...
        static int hello_count = 0;  // Keep track of how many times we've sent "Hello Server"
        ...
        if (hello_count < 5) {
            ...
            // After 5 "Hello Server" messages, send light sensor data
            char buffer[50];
            snprintf(buffer, sizeof(buffer), "Light Intensity: %.2f%%", state->data.light_intesity);
            DEBUG_printf("Writing %ld bytes to client\n", strlen(buffer));
            err_t err = tcp_write(tpcb, buffer, strlen(buffer), TCP_WRITE_FLAG_COPY);
            if (err != ERR_OK) {
                DEBUG_printf("Failed to write data %d\n", err);
                return tcp_server_result(arg, -1);
            }
...
err_t tcp_server_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err) {
    ...
    if (p->tot_len > 0) {
        ...
        char msg_buffer[256] = {0};  // Adjust size as needed
        
        // Copy the received data into our buffer
        pbuf_copy_partial(p, msg_buffer, p->tot_len > 255 ? 255 : p->tot_len, 0);
        
        // Print the received message to serial monitor
        printf("Received message: %s\n", msg_buffer);

        // Original buffer handling for command processing
